...
import { getServiceRatingsRequest, addServiceRatingRequest, bsnsDetailsRequest } from '../../redux/actions';

const { width, height } = Dimensions.get('window');

export const ServiceRating = ({ modalizeRef, modalData, companyId }) => {
  ...
  const getRatings = () => {
    ...
    const cbSuccess = async message => {
      // console.log('Service Ratings are ==> ', message?.Response);
  ...
  const getBusinessDetails = () => {
    ...
    dispatch(bsnsDetailsRequest(companyId, cbSuccess, cbFailure));
  };

 //console.log(' Service  ', modalData, ' id ', companyId)
  ...
  const submitWork = () => {
    if (review === '') {
      ...
      formData.append('serviceId', modalData?.ServiceId);
      ...
      dispatch(
        addServiceRatingRequest(params, userData?.Token, cbSuccess, cbFailure),
      );
    }
  };
      ...
      <ImageBackground source={appImages.splashBg} style={styles.main}>
        ...
        <ScrollView
          ...
          <View style={styles.container}>
            <Text style={styles.heading}>{modalData?.ServiceName}</Text>
            <ImageBackground
              ...
              <Image
                source={{
                  uri:
                    modalData?.ServiceImage_1 ||
                    modalData?.ServiceImage_2 ||
                    modalData?.ServiceImage_3,
                }}
            ...
            <View style={styles.ratingContainer}>
              <Rating
                ...
                imageSize={20}
                tintColor={colors.g9}
                startingValue={Math.round(modalData?.ServiceRatingPercentage)}
              />
              <Text style={styles.ratingTxtStyle}>({modalData?.ServiceNumberOfRating})</Text>
            </View>
            <Image
